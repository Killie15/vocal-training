...
async function sendVocalIdentification(frequency) {
  ...
  try {
    ...
    if (result.matched) {
      feedback.style.color = 'var(--neon-green)';
      feedback.innerHTML = `<i class="fa-solid fa-circle-check"></i> Welcome back, <strong>${escapeHtml(result.user.username)}</strong>!<br><span style="font-size: 0.75rem; color: var(--text-muted);">Voice matched within ${result.distance} semitones. Logging in...</span>`;
    ...
    if (closestUser && minDistance <= 6.0) { // Limit match to within 6 semitones (1/2 octave)
      ...
      saveCurrentUserLocal(currentUser);
      feedback.style.color = 'var(--neon-green)';
      feedback.innerHTML = `<i class="fa-solid fa-circle-check"></i> (Offline) Welcome back, <strong>${escapeHtml(closestUser.username)}</strong>!<br><span style="font-size: 0.75rem; color: var(--text-muted);">Voice matched within ${Math.round(minDistance * 10) / 10} semitones. Logging in...</span>`;
      setTimeout(() => {
        updateUIWithUser();
        feedback.style.display = 'none';
      }, 1500);
...
// POST progress to SQLite database if this is a real level
    if (levelId > 0) {
      ...
      addToOfflineQueueLocal({
        type: 'progress',
        queueId: crypto.randomUUID ? crypto.randomUUID() : 'q_' + Date.now() + '_' + Math.random().toString(36).substr(2, 9),
        level_id: levelId,
        stars_earned: report.stars,
        score: report.score,
        pitch_accuracy_pct: report.accuracy,
    ...
    for (const item of queue) {
      try {
        ...
        if (item.type === 'progress') {
          requestBody.queue_id = item.queueId || item.queue_id;
        }
